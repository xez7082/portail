// Méthode de secours infaillible : On charge les bibliothèques depuis le CDN officiel de HA (contournement CSP)
...
litScript.textContent = `
  ...
  const DEFAULT_CONFIG = {
    titre_principal: "Portail Maison", taille_texte: 17,
    couleur_de_fond: "#1c2431", couleur_cartes: "#27303f", couleur_accent: "#3d8de0",
    pages_version: 1,
    menus: [
  ...
  class PortailCard extends LitElement {
    ...
    get currentSub() { const m = this.currentMenu; return (m && m.sous_menus) ? (m.sous_menus[this._activeSub] || m.sous_menus[0] || null) : null; }

    render() {
      const sub = this.currentSub;
      const src = sub && sub.chemin ? \`\${sub.chemin}?v=\${this._config.pages_version || 1}\` : "";
      return html\`
        <header>
        ...
        <main>
          ...
          <div id="content">
            \${sub && sub.chemin ? html\`<iframe src="\${src}"></iframe>\` : html\`<div id="placeholder"><div class="big">\${sub ? sub.nom : this._config.titre_principal}</div><div class="small">\${sub ? "Chemin HTML manquant." : "Choisissez une rubrique."}</div></div>\`}
          </div>
        </main>\`;
    }

    _changeFont(d) { this._config.taille_texte = Math.min(24, Math.max(14, (this._config.taille_texte || 17) + d)); this.style.fontSize = \`\${this._config.taille_texte}px\`; this.dispatchEvent(new CustomEvent('config-changed', { detail: { config: this._config }, bubbles: true, composed: true })); }
  ...
  class PortailEditor extends LitElement {
    ...
    saveConfig() { this.dispatchEvent(new CustomEvent('config-changed', { detail: { config: this._draftConfig }, bubbles: true, composed: true })); }
  }
