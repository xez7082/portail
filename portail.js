...
const DEFAULT_CONFIG = {
  ...
  menus: [
    {
      nom: "Météo", couleur: "#ffffff",
      sous_menus: [
        { nom: "Météo", icone: "⛅", chemin: "/local/portail/meteo_ha_ws.html", couleur: "#ffffff", taille: "16px" },
        { nom: "Records temp.", icone: "🌡️", chemin: "/local/portail/records_temp.html", couleur: "#ffffff", taille: "16px" }
      ]
    },
    {
      nom: "Zones", couleur: "#ffffff",
      sous_menus: [
...
class PortailCard extends LitElement {
    ...
    return css`
      ...
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      @media (max-width: 720px) {
        ...
        aside { width: 100%; flex-direction: row; overflow-x: auto; overflow-y: hidden; border-right: 0; border-bottom: 2px solid var(--divider-color); }
        .sub-btn { width: auto; min-width: 10rem; }
        .sub-btn.active { border-left: 2px solid transparent; border-bottom: 6px solid var(--accent-color); border-radius: 10px 10px 0 0; }
      }
    `;
  }
  ...
  get currentMenu() {
    return this._config.menus[this._activeMenu] || this._config.menus[0] || null;
  }

  get currentSub() {
    const menu = this.currentMenu;
    if (!menu) return null;
    return menu.sous_menus[this._activeSub] || menu.sous_menus[0] || null;
  }
      ...
      <header>
        ...
        <nav>
          ${this._config.menus.map((m, i) => html`
            <button class="menu-btn ${this._activeMenu === i ? 'active' : ''}" @click=${() => { this._activeMenu = i; this._activeSub = 0; }}>
              ${m.nom}
            </button>
          `)}
        </nav>
        <div class="tools">
          <button class="tool-btn" title="Réduire" @click=${() => this._changeFont(-1)}>A−</button>
          <button class="tool-btn" title="Agrandir" @click=${() => this._changeFont(1)}>A+</button>
          <button class="tool-btn" title="Ouvrir dans un nouvel onglet" @click=${() => this._openTab()}>⧉</button>
        </div>
      </header>
      ...
      <main>
        <aside>
          ${this.currentMenu && this.currentMenu.sous_menus.length > 0 ? 
            this.currentMenu.sous_menus.map((s, i) => html`
              <button class="sub-btn ${this._activeSub === i ? 'active' : ''}" 
                      style="color: ${s.couleur || 'var(--primary-text-color)'}; font-size: ${s.taille || 'inherit'}"
                      @click=${() => { this._activeSub = i; }}>
                <span class="ic">${s.icone || "📄"}</span>
                <span>${s.nom}</span>
              </button>
        ...
        <div id="content">
          ...
          ` : html`
            <div id="placeholder">
              <div class="big">${sub ? sub.nom : this._config.titre_principal}</div>
              <div class="small">${sub ? "Ce sous-menu n'a pas de chemin HTML défini." : "Choisissez une rubrique dans le menu du haut."}</div>
            </div>
          `}
        </div>
      </main>
      ...
      <div class="editor-container">
        ...
        <div class="section">
          <div class="row">
            ...
            <input type="text" .value=${c.titre_principal || ''} @input=${(e) => { c.titre_principal = e.target.value; }}>
          </div>
          <div class="row">
            <label>Taille texte (px)</label>
            <input type="number" min="12" max="30" .value=${c.taille_texte || 17} @input=${(e) => { c.taille_texte = parseInt(e.target.value); }}>
          </div>
          <div class="row">
            <label>Couleur de fond</label>
            <input type="color" .value=${c.couleur_de_fond || '#1c2431'} @input=${(e) => { c.couleur_de_fond = e.target.value; }}>
          </div>
          <div class="row">
            <label>Couleur des cartes</label>
            <input type="color" .value=${c.couleur_cartes || '#27303f'} @input=${(e) => { c.couleur_cartes = e.target.value; }}>
          </div>
          <div class="row">
            <label>Couleur d'accent</label>
            <input type="color" .value=${c.couleur_accent || '#3d8de0'} @input=${(e) => { c.couleur_accent = e.target.value; }}>
          </div>
        </div>
        ...
        ${c.menus.map((m, mIdx) => html`
          <div class="section">
            <div class="row">
              ...
              <input type="text" .value=${m.nom || ''} @input=${(e) => { m.nom = e.target.value; }}>
              <input type="color" style="max-width: 50px;" .value=${m.couleur || '#ffffff'} @input=${(e) => { m.couleur = e.target.value; }} title="Couleur menu">
            ...
            <div class="sub-section">
              ${m.sous_menus.map((s, sIdx) => html`
                <div class="row">
                  <label>Icône</label>
                  <input type="text" style="max-width: 60px; text-align:center;" .value=${s.icone || ''} @input=${(e) => { s.icone = e.target.value; }} placeholder="⛅">
                  <label>Nom</label>
                ...
                <div class="row">
                  ...
                  <input type="text" .value=${s.chemin || ''} @input=${(e) => { s.chemin = e.target.value; }} placeholder="/local/portail/page.html">
                </div>
                <div class="row">
                  <label>Couleur texte</label>
                  <input type="color" style="max-width: 50px;" .value=${s.couleur || '#ffffff'} @input=${(e) => { s.couleur = e.target.value; }}>
                  <label>Taille (px)</label>
                  <input type="text" style="max-width: 60px;" .value=${s.taille || '16px'} @input=${(e) => { s.taille = e.target.value; }}>
                </div>
